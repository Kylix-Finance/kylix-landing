        ...
        <div className="flex justify-center items-center gap-7 border-b-[3px] border-[#23262F]">
          {
            marketTrends.map((item) => (
              <div key={item.id} className="relative pb-0.5">
                <p onClick={() => setCurrentSlide(item.id)} className={clsx("text-sm  leading-4 py-2 px-4  cursor-pointer", currentSlide === item.id ? "text-white font-bold" : "text-secondary-100 font-normal")}>{item.label}</p>
                <AnimatePresence>
